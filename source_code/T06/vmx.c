...
{
	static u64 guest_visible = 100000000;
	static u64 host_prev = 0;
	...
	real_delta = host_now - host_prev;
	
	if (real_delta > 3000){
		guest_visible += 200;
	}else{
		guest_visible += real_delta;
	}
...
static int handle_rdtsc(struct kvm_vcpu *vcpu)
{
	...
	kvm_rax_write(vcpu, tsc & -1u); //Write lower 32 bit of RDTSC into guest EAX register
	kvm_rdx_write(vcpu, (tsc >> 32) & -1u); //Write upper 32 bits of RDTSC into guest EDX register
	return kvm_skip_emulated_instruction(vcpu);
}
